...
export default function SettingsPage() {
  ...
  function addMarket() {
    ...
    if (v && !detailMarkets.includes(v)) {
      setDetailMarkets((p) => [...p, v]);
      setNewMarket("");
    }
  }
      ...
      <div className="space-y-5">
        ...
        >
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={addCat}
              className="bg-gray-700 hover:bg-gray-600 text-gray-200 rounded-lg px-4 py-2 text-sm transition-colors"
            >
              + Adicionar
            </button>
          </div>
              ...
              <div key={cat} className="bg-gray-800/60 rounded-lg p-4">
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {(est[cat] ?? []).map((place, idx) => (
                    <span
                      key={idx}
                      className="flex items-center gap-1 bg-gray-700 text-gray-300 text-sm rounded-full px-3 py-1"
                    >
                      {place}
                      <button
                        onClick={() => removeEst(cat, idx)}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => addEst(cat)}
                    className="bg-gray-700 hover:bg-gray-600 text-gray-300 rounded-lg px-3 py-1.5 text-sm transition-colors"
                  >
                    +
                  </button>
                </div>
        ...
        {/* ── Mercados com nota detalhada ──────────────────────────────────── */}
        <Section
          title="Mercados com nota detalhada"
          description="Nesses mercados o bot lê cada item da nota e divide automaticamente entre Alimentação, Mercado, Cosméticos, etc."
        >
          <div className="flex flex-wrap gap-2 mb-4">
            {[...new Set([...detailMarkets])].map((m) => (
        ...
        <Section
          ...
          description='Quando o usuário diz "pix", "débito" ou "conta" sem mencionar um cartão específico, o bot usa esse nome.'
        >
        ...
        <Section
          ...
          description="Texto adicionado ao final do prompt. Use para ensinar contextos pessoais: nomes de pessoas, estabelecimentos incomuns, regras específicas."
        >
          <textarea
        ...
        {/* ── Usuários ativos ───────────────────────────────────────────────── */}
        <Section
          title="Usuários que já usaram o bot"
          description="Extraído automaticamente do histórico de transações. Para adicionar ou remover acesso, atualize TELEGRAM_USER_IDS nas variáveis do Fly.io."
        >
